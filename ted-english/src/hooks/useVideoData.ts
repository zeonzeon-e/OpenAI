...
const TED_YOUTUBE_CHANNEL_ID = "UCAuUUnT6oDeKwE6v1NGQxug";
...
const extractVocabulary = (sentence: string) => {
  const words = Array.from(
    new Set(sentence.toLowerCase().match(/[a-z]{6,}/g) ?? [])
  );
...
const buildTranscriptFromDescription = (
  title: string,
  description: string
): TranscriptSegment[] => {
        ...
        vocabulary: [
          {
            term: "explore",
            definition:
              "explore의 의미를 사전에서 확인하고 직접 문장을 만들어 보세요.",
            example:
              "Try to explore new TED ideas and summarize them in English.",
          },
        ],
      },
    ];
  ...
  return Array.from(doc.getElementsByTagName("entry"))
    .map((entry) => {
      const getText = (selector: string) =>
        entry.querySelector(selector)?.textContent?.trim() ?? "";
      const getAttr = (selector: string, attribute: string) =>
        entry.querySelector(selector)?.getAttribute(attribute) ?? "";
...
const mergeWithLocal = (video: VideoDetail): VideoDetail => {
  ...
  return {
    ...
    learningObjectives:
      local.learningObjectives.length > 0
        ? local.learningObjectives
        : video.learningObjectives,
    transcript:
      local.transcript.length > 0 ? local.transcript : video.transcript,
  };
};
  ...
  useEffect(() => {
    ...
    const fetchVideos = async () => {
      try {
        ...
        if (!response.ok) {
          throw new Error(
            `YouTube 피드를 불러오지 못했습니다: ${response.status}`
          );
        }

        const payload = await response.text();
        const parsed = parseYoutubeFeed(payload).filter((video) =>
...
const TED_TALKS_ENDPOINT =
  "https://tedcdnpi-a.akamaihd.net/api/tedtalks/v1/?language=en&limit=50";
...
const deriveYoutubeId = (talk: TedTalkApi) => {
  return (
    ...
    talk.resources?.external
      ?.map((resource) => extractYoutubeId(resource.uri))
      .find(Boolean) ??
    talk.external
      ?.map((resource) => extractYoutubeId(resource.uri))
      .find(Boolean) ??
...
const mapTalkToVideo = (talk: TedTalkApi): VideoDetail | null => {
  const id =
    talk.slug ?? (talk.id !== undefined ? `ted-${talk.id}` : undefined);
  ...
  const durationSeconds =
    talk.duration ?? talk.video?.duration ?? talk.video?.media_duration ?? null;
  ...
  return {
    ...
    youtubeId,
    publishedAt:
      talk.published_at ?? talk.release_date ?? talk.recorded_at ?? "",
    learningObjectives: [],
    transcript: [],
  };
};
